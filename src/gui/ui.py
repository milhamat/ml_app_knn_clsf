...
class Interface:
    ...
    def lunch(self):
        screen = Tk()
        screen.title("SMarket")
        # Adjust size
        screen.geometry("400x400")
        # set minimum window size value
        screen.minsize(150, 150)
        # set maximum window size value
        screen.maxsize(250, 300)
        ...
        button.grid(row=len(name_feature), column=0, sticky = W, padx=5, pady=5)

        rd_button = Button(screen, text="Random", command=gen_random)
        rd_button.grid(row=len(name_feature), column=1, sticky = W, padx=5, pady=5)

        predict_result = Label(screen, text="Predict Result :")
        predict_result.grid(row=len(name_feature)+2, column=1, sticky = W, pady=5)
